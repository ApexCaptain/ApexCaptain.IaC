...
<ServerSettings>
	...
	<property name="Region"								value="${Region}"/>								<!-- The region this server is in. Values: NorthAmericaEast, NorthAmericaWest, CentralAmerica, SouthAmerica, Europe, Russia, Asia, MiddleEast, Africa, Oceania -->
	<property name="Language"							value="${Language}"/>							<!-- Primary language for players on this server. Values: Use any language name that you would users expect to search for. Should be the English name of the language, e.g. not "Deutsch" but "German" -->

	<!-- Networking -->
	<property name="ServerPort"							value="${ServerPort}"/>							<!-- Port you want the server to listen on. Keep it in the ranges 26900 to 26905 or 27015 to 27020 if you want PCs on the same LAN to find it as a LAN server. -->
	...
	<property name="TelnetPassword"						value="${TelnetPassword}"/>						<!-- Password to gain entry to telnet interface. If no password is set the server will only listen on the local loopback interface -->
	...
	<property name="ServerAllowCrossplay"				value="${ServerAllowCrossplay}"/>				<!-- Enables/Disables crossplay, crossplay servers will only be found in searches and joinable if sanctions are not ignored, and have a default or fewer player slot count  -->
	...
	<property name="MaxUncoveredMapChunksPerPlayer"		value="${MaxUncoveredMapChunksPerPlayer}"/>		<!-- Override how many chunks can be uncovered on the in-game map by each player. Resulting max map file size limit per player is (x * 512 Bytes), uncovered area is (x * 256 m²). Default 131072 means max 32 km² can be uncovered at any time -->
	...
	<property name="SaveDataLimit"						value="${SaveDataLimit}"/>						<!-- The maximum disk space allowance for each saved game in megabytes (MB). Saved chunks may be forcibly reset to their original states to free up space when this limit is reached. Negative values disable the limit. -->

	<!-- GAMEPLAY -->

	<!-- World -->
	<property name="GameWorld"							value="${GameWorld}"/>							<!-- "RWG" (see WorldGenSeed and WorldGenSize options below) or any already existing world name in the Worlds folder (currently shipping with e.g. "Navezgane", "Pregen06k01", "Pregen06k02", "Pregen08k01", "Pregen08k02", ...) -->
	...
	<property name="WorldGenSize"						value="${WorldGenSize}"/>						<!-- If RWG, this controls the width and height of the created world. Officially supported sizes are between 6144 and 10240 and must be a multiple of 2048, e.g. 6144, 8192, 10240. --> 
	<property name="GameName"							value="${GameName}"/>							<!-- Whatever you want the game name to be (allowed [A-Za-z0-9_-. ]). This affects the save game name as well as the seed used when placing decoration (trees etc.) in the world. It does not control the generic layout of the world if creating an RWG world -->
	...
	<property name="BiomeProgression"					value="${BiomeProgression}"/>					<!-- Enables biome hazards and loot stage caps to promote biome progression. Loot stage caps are increased by completing biome challenges. -->
	<property name="StormFreq"							value="${StormFreq}"/>							<!-- Adjusts the frequency of storms. 0% turns them off. Vanilla values: 0, 50, 100, 150, 200, 300, 400, 500 -->
	<property name="DeathPenalty"						value="${DeathPenalty}"/>						<!-- Penalty after dying. 0 = Nothing. 1 = Default: Classic XP Penalty.  2 = Injured: You keep most of your de-buffs. Food and Water is set to 50% on respawn. 3 = Permanent Death: Your character is completely reset. You will respawn with a fresh start within the saved game. -->
	...
	<property name="BedrollDeadZoneSize"				value="${BedrollDeadZoneSize}"/>				<!-- Size (box "radius", so a box with 2 times the given value for each side's length) of bedroll dead zone, no zombies will spawn inside this area, and any cleared sleeper volumes that touch a bedroll deadzone will not spawn after they've been cleared. -->
	...
	<property name="AllowSpawnNearFriend"				value="${AllowSpawnNearFriend}"/>				<!-- Can new players joining the server for the first time select to join near any friend playing at the same time? 0 = Disabled, 1 = Always, 2 = Only near friends in forest biome -->
	...
	<property name="ServerMaxAllowedViewDistance"		value="${ServerMaxAllowedViewDistance}" />		<!-- Max view distance a client may request (6 - 12). High impact on memory usage and performance. -->
	...
	<property name="LandClaimExpiryTime"				value="${LandClaimExpiryTime}"/>				<!-- The number of real world days a player can be offline before their claims expire and are no longer protected -->
	...
	<property name="QuestProgressionDailyLimit"			value="${QuestProgressionDailyLimit}"/>			<!-- Limits the number of quests that contribute to quest tier progression a player can complete each day. Quests after the limit can still be completed for rewards. -->
